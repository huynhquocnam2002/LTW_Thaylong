...
<body>
<%Producer producer = ProducerDAO.getProducer(request.getParameter("id"));%>
...
<section id="admin-content" class="section">
    <div class="container" style="position: relative;">
        <h1 class="tab-name" style=" overflow: hidden; position: absolute; left: 100px;top:0px; z-index: 999;  white-space: nowrap;
            overflow: hidden; color:var(--black4)">Sửa thông tin nhà sản xuất</h1>
        <div id="rightContainer">
            <div class="right-tab add-new-category add-tab">
                <div class="right-tab-container" style="margin-top: 0">
                    <form action="/InfoProducerServlet" method="post" enctype="multipart/form-data" style="margin-top: 0 !important;padding-bottom: 40px">
                        <input type="hidden" name="id" value="<%=producer.getId()%>">
                        <table class="add-new-product-table">
                            <tr class="table-row">
                                <td class="label-col">
                                    <label for="new-category-name">Tên nhà sản xuất:</label>
                                </td>
                                <td class="input-col">
                                    <input
                                            type="text" value="<%=producer.getName()%>"
                                            name="name"
                            ...
                            <tr class="table-row">
                                ...
                                <td class="input-col">
                                    <select name="status"
                                        ...
                                        <option value="-1" class="new-category-status-option" <%=producer.getStatus()==-1?"selected":""%>>
                                            Đã ẩn
                                        </option>
                                        <option value="1"
                                                class="new-category-status-option"
                                                <%=producer.getStatus()==1?"selected":""%>>
                                            Đang hoạt động
                                        </option>
                                    </select>
                                </td>
                            ...
                            <tr class="table-row">
                                ...
                                <td class="input-col">
                                    <img src="<%=producer.getImg()%>" id="img-demo" style="width: 200px; height: auto" alt="">
                                    <input
                                            type="file" value="<%=producer.getImg()%>"
                                            name="img"
