...
<!--Nhãn Hiệu-->
 <% List<Chinhsachbaomat> listchinhsach = FooterDAO.getChinhsachbaomat();%>


<section class="section baomat">
  <div class="container">
    <div class="content">
      <h1 class="content-title"> CHÍNH SÁCH BẢO MẬT CỦA JCSHOP</h1>
      <h3>Xin vui lòng đọc chính sách bảo mật một cách cẩn thận, để có được một sự hiểu biết rõ ràng về cách thức thông tin cá nhân của bạn được thu thập, sử dụng, bảo vệ.</h3>
      <%  for (int i = 0; i <listchinhsach.size() ; i++) {%>
      <div class="content-linetext">
        <p class="content-linetext_cap1"><%=listchinhsach.get(i).getId()%>. <%=listchinhsach.get(i).getName()%></p>
        <p class="content-linetext_cap2"><%=listchinhsach.get(i).getText()%></p>
      </div>
      <%}%>
      <br><p class="content-camon" style="text-align: center;"> Xin Chân Thành Cảm Ơn !</p>
