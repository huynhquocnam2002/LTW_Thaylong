...
<%@ page import="vn.edu.hcmuaf.fit.model.Voucher_footer" %>
...
<!--Nhãn Hiệu-->
  <% List<Voucher_footer> listvoucher = FooterDAO.getVoucher_footer(); %>
<section class="section phieuquatang">
  <div class="container">
    <div class="content">
      <h1 class="content-title">THU THẬP VOUCHER MIỄN PHÍ VẬN CHUYỂN</h1>
      <ul>
        <% for (int i = 0; i < listvoucher.size(); i++) {%>
        <li>
          <div class="content-voucher">
            <img class="image_nhanhieu" src="<%=listvoucher.get(i).getImage()%>" alt="" style="background-color: #BE1E2D;">
            <div>
              <h2><%=listvoucher.get(i).getName()%></h2>
              <P><%=listvoucher.get(i).getText()%></P>
            </div>
          </div>
        </li>
        <%} %>
