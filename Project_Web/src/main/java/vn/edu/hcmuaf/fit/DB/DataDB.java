...
public class DataDB {
    ...
    public static Set<Product> getProducts() throws SQLException, ClassNotFoundException {
        ...
        while (rs.next()) {
            res.add(new Product(rs.getString(1), rs.getString(2), rs.getString(4), rs.getLong(3), rs.getString(5), rs.getInt(6)));
        }
        return res;
    }
    ...
    public static boolean register(String id, String email, String pass) throws SQLException, ClassNotFoundException {
        ...
        if (db.getStatement().executeUpdate("insert into user values ('"+id+"','image/user/none.png','user "+id+"',null,'"+email+"','"+pass+"',null,null,0,'"+dtf.format(now)+"');")==0) return false;
        return true;
    }

    public static boolean activeUser(User u) throws SQLException, ClassNotFoundException {
        DataDB db= new DataDB();
        if (db.getStatement().executeUpdate("update user set status=1 where id='"+u.getId()+"';")==0) return false;
        return true;
    }

    public static void main(String[] args) throws SQLException, ClassNotFoundException {
        System.out.println(getUserByEmail("thuan@gmail.com"));
        System.out.println(getCartItems(getUserByEmail("thuan@gmail.com")).size());
    }
}
